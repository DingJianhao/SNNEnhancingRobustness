...
class EarlyStopping:
    ...
    def __init__(self, patience=50, verbose=False, delta=0, dirn='checkpoint', ckptname='ckpt'):
        """
        Args:
    ...
    def __call__(self, val, model, epoch):
        ...
        if self.best_score is None:
            ...
            self.save_checkpoint(model, val, epoch, self.dirn, self.ckptname)
    ...
    def save_checkpoint(self, network, val, epoch, dirn='checkpoint', ckptname='ckpt'):
        ...
        if not os.path.isdir(dirn):
            os.mkdir(dirn)
        torch.save(state, './%s/%s.pth' % (dirn, ckptname))
        self.val_min = val
